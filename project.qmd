...
```{r}
datap <- "data"
renew_21 <- read_csv(file.path(datap, "renew-use-2021.csv"))  |> clean_names()
renew_22 <- read_csv(file.path(datap, "renew-use-2022.csv"))  |> clean_names()
renew_23 <- read_csv(file.path(datap, "renew-use-2023.csv"))  |> clean_names()

total_21 <- read_csv(file.path(datap, "total-use-2021.csv"))  |> clean_names()
total_22 <- read_csv(file.path(datap, "total-use-2022.csv"))  |> clean_names()
total_23 <- read_csv(file.path(datap, "total-use-2023.csv"))  |> clean_names()

price    <- read_csv(file.path(datap, "av-energy-price-2021-2023.csv")) |> clean_names()
ev_23    <- read_csv(file.path(datap, "ev-registrations-by-state-2023.csv")) |> clean_names()

normalize_state <- \(x) str_to_title(str_trim(x))
fix_states <- \(df) df |> mutate(state = normalize_state(state))
renew_21 <- fix_states(renew_21); renew_22 <- fix_states(renew_22); renew_23 <- fix_states(renew_23)
total_21 <- fix_states(total_21); total_22 <- fix_states(total_22); total_23 <- fix_states(total_23)
price    <- fix_states(price)
ev_23    <- fix_states(ev_23)
